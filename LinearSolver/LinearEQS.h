...
//#endif
      ...
      {
         A->setDOF(dof_n);  // For different processes with different DOF of OPDE. _new. 10/2008. WW
      }
      void setKnownX_i(const long i, const double x_i);
      double X(const long i) const
      {
         return x[i];
      }
      double RHS(const long i) const
      {
         return b[i];
      }
      double NormX();
   ...
   private:
      ...
      double Norm(const double *xx)
      {
         return sqrt(dot(xx, xx));
      }
